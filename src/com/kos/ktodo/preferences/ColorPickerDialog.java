...
/**
 * Color picker by Yougli. Taken from
 * <a href="http://www.yougli.net/android/a-photoshop-like-color-picker-for-your-android-application/">here</a>, with some minor modifications.
 */
public class ColorPickerDialog extends Dialog {
	...
	private static class ColorPickerView extends View {
		...
		ColorPickerView(final Context c, final OnColorChangedListener l, final int color, final int defaultColor) {
			super(c);
			mListener = l;
		...
		private int getCurrentMainColor() {
			final int translatedHue = 255 - (int) (mCurrentHue * 255 / 360);
			int index = 0;
		...
		private void updateMainColors() {
			final int mainColor = getCurrentMainColor();
			int index = 0;
			final int[] topColors = new int[256];
			for (int y = 0; y < 256; y++) {
				for (int x = 0; x < 256; x++) {
					if (y == 0) {
		...
		protected void onDraw(final Canvas canvas) {
			...
			for (int x = 0; x < 256; x++) {
				...
				final Shader shader = new LinearGradient(0, 50, 0, 306, colors, null, Shader.TileMode.REPEAT);
			...
			if (Color.red(mCurrentColor) + Color.green(mCurrentColor) + Color.blue(mCurrentColor) < 384)
				...
				mPaint.setColor(Color.BLACK);
			canvas.drawText(getResources().getString(R.string.prefs_bg_color_confirm), 74, 340, mPaint);
			...
			if (Color.red(mDefaultColor) + Color.green(mDefaultColor) + Color.blue(mDefaultColor) < 384)
				...
				mPaint.setColor(Color.BLACK);
			canvas.drawText(getResources().getString(R.string.prefs_default_color_confirm), 202, 340, mPaint);
		}
		...
		public boolean onTouchEvent(final MotionEvent event) {
			...
			if (x > 10 && x < 266 && y > 0 && y < 40) {
				...
				final int transX = mCurrentX - 10;
				final int transY = mCurrentY - 60;
				final int index = 256 * (transY - 1) + transX;
	...
	protected void onCreate(final Bundle savedInstanceState) {
		...
		setTitle(R.string.choose_color_dlg_title);
	}
}
